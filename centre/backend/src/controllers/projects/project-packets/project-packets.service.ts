...
export class ProjectPacketsService {
  ...
  async queryPacketsAfter(projectName: string, query: QueryPacketAfterTimeDto) {
    const project = await this.projectModel.findOne({ name: projectName });
    if (!project)
      throw new NotFoundException([], `Not found project ${projectName}`);
    const { from } = query;
    ...
    const agg = findPacketsAggregation(filter, query.queryDistinct, true);
    const packets = await this.rawPacketModel.aggregate(agg).allowDiskUse(true);
    ...
    if (!query.minimal) return packets;
    return minimizePacketOutput(packets);
  }
}
